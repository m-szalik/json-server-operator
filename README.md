 ...
 * Make
 * Go 1.20+
 * Empty cluster (can be for example Kind)
 * kubectl - configured with admin access to the cluster

There are few e2e test cases in [e2e_test.go](test/e2e/e2e_test.go).
...
4. Helm package.
5. More e2e tests
6. E2e tests integrated with build pipeline. 
7. Complete integration with [OLM - Operator Lifecycle Manager](https://olm.operatorframework.io/).
